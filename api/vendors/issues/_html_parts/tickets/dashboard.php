                    ...
                    <tr>
                        ...
                        foreach ($stat_order as $key => $val) {
                                    ...
                                    if ($ticket['status'] == $val) {
                                        ...
                                        <div class="issue-box <?= $disabled == true ? 'disable-sort-item' : '' ?>" data-have-status="<?= $val ?>" data-ticket-id="<?= $ticket['ticket_id'] ?>" style="border-left:5px solid <?= $ticket['color'] ?>">
                                            <div style="width:80%">
                                                ...
                                                <span class="text-info glyphicon glyphicon-user"> <?= word_limiter($ticket['assignee'], 100) ?></span>
                                                <div class="ticket-subject"><span><strong><?= word_limiter($ticket['subject'], 100) ?></strong></span></div></div>
                            ...
                            <!--<div><span class="timecreated" data-toggle="tooltip" title="<?= $this->lang_php['assignee'] ?>: <?= $ticket['fullname'] ?>"><span class="glyphicon glyphicon-user"></span> <?= $ticket['fullname'] ?></span></div>-->
                                            ...
                                            <?php
                                            $ticket_seconds = strtotime(date('d-m-Y H:i:s', time())) - strtotime(date('d-m-Y H:i:s', $ticket['timecreated']));
                                            $ticket_days = floor($ticket_seconds / 86400);
                                            $ticket_hours = floor(($ticket_seconds - ($ticket_days * 86400)) / 3600);
                                            $ticket_minutes = floor(($ticket_seconds - ($ticket_days * 86400) - ($ticket_hours * 3600)) / 60);

                                            if ($ticket_hours == 0) {
                                                $ticket_time_string = $ticket_minutes . ' minutes ago';
                                            } else if ($ticket_hours > 48) { 
                                                $ticket_time_string = $ticket_days . ' days ago';
                                            } else {
                                                $ticket_time_string = $ticket_hours . ' hours ago';
                                            }
                                            ?>
                                            <div><span class="timecreated" data-toggle="tooltip" title="Total Time"><span class="glyphicon glyphicon-hourglass"></span> <?php echo $ticket_time_string; ?></span></div>                                                                                      
                                            ...
                                            <div class="profile-img">
                                                ...
                                                <?php } else { ?>
                                                    <a href="<?= base_url('profile/' . $ticket['username']) ?>"><img src="<?= base_url(returnImageUrl($ticket['assignee_image'])) ?>" class="profile-img img-circle" data-toggle="tooltip" title="Issue By: <?= $ticket['assignee'] ?>"></a>
                                                    <?php } ?>
                                            </div>
                                        </div>
                                        <?php
