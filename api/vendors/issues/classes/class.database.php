...
//Filter END!
            ...
            $result = $this->query("SELECT tickets.ticket_id, tickets.id, tickets.subject,tickets.assignee, "
                    . "tickets.send, tickets.message_uid, tickets.message_from_email, tickets.message_from_name,"
                    . " tickets.status, tickets.assignee as assignee_id, priority_colors.color,"
                    . " tickets.timecreated, users.fullname, users.username, users.image as assignee_image,"
                    . " usersA.id as addedby, ticket_types.name as type_name,"
                    . " tickets.duedate FROM tickets LEFT JOIN ticket_priority ON "
                    . "ticket_priority.id = tickets.priority INNER JOIN projects ON "
                    . "projects.id=tickets.project LEFT JOIN priority_colors ON "
                    . "priority_colors.for_id = ticket_priority.id LEFT JOIN users ON "
                    . "users.id = tickets.assignee LEFT JOIN users as usersA "
                    . "ON usersA.id = tickets.addedby LEFT JOIN ticket_types ON"
                    . " ticket_types.id = tickets.type $fast_watched_join WHERE"
                    . " tickets.for_account=" . ACCOUNT_ID . " AND "
                    . "projects.name='$this->project_name' "
                    . "$where $fast_assign_to_me $fast_watched_where ORDER BY id DESC");
            if ($result !== false) {
                while ($row = $result->fetch_assoc()) {
                    $arr['tickets'][] = $row;
                }
