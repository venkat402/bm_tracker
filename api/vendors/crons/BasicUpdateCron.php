...
//        die;
    ...
    public function updateTime() {
        ...
        if (!$this->tracker_conn->query("UPDATE domains_info_note SET note_value = '$date' where note_key = 'BasicUpdateCron.php' and note_type='crons'")) {
            var_dump($this->tracker_conn->error);
        }
    }

    public function updateIssues() {
        $this->tracker_conn->query("SELECT count(*) as count FROM bulkmail_bugs2.tickets WHERE status = '1'");
        $row = $result->fetch_assoc();
        $list_count = $row['count'];
        if (!$this->tracker_conn->query("UPDATE domains_info_note SET note_value = '$date' where note_key = 'BasicUpdateCron.php' and note_type='crons'")) {
            var_dump($this->tracker_conn->error);
	...
	WHERE domain_name = '$domain_name'");
        if (!$result) {
            var_dump("$domain_name updateing domain failed=" . $this->tracker_conn->error . "<br>");
        } else {
...
echo '<pre>';
$BasicUpdateCron = new BasicUpdateCron();
$BasicUpdateCron->updateServers();
