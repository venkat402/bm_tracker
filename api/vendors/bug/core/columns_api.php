...
function columns_get_all_active_columns() {
    $t_columns = array_merge(
            columns_get_standard(), array_keys(columns_get_plugin_columns()), columns_get_custom_fields()
    );
...
function print_column_status(BugData $p_bug, $p_columns_target = COLUMNS_TARGET_VIEW_PAGE) {
    ...
    printf('<span title="%s">%s</span>', get_enum_element('resolution', $p_bug->resolution, $t_current_user, $p_bug->project_id), get_enum_element('status', $p_bug->status, $t_current_user, $p_bug->project_id)
    );

    # print handler user next to status
    if ($p_bug->handler_id > 0 && ON == config_get('show_assigned_names', null, $t_current_user, $p_bug->project_id) && access_can_see_handler_for_bug($p_bug)) {
...
function print_column_summary(BugData $p_bug, $p_columns_target = COLUMNS_TARGET_VIEW_PAGE) {
    ...
    echo '<td class="column-summary"><a href="view.php?id=' . $p_bug->id . '"><font color="black">' . $t_summary . '</a></td>';
}
