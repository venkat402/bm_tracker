...
echo '<pre>';
class MoveClientsCron extends CI_Controller {
    ...
    public function getMoveClientRecord() {
        ...
        $result = $this->db->query("select * from moveceo where message LIKE '%added successfully%' LIMIT 1");
        if (!$result) {
    ...
    public function moveClientErrorLog($errorLog, $ceoData) {
        $this->load->model('Home_model');
        $id = $ceoData['id'];
        $result = $this->db->query("UPDATE moveceo SET message='$errorLog' WHERE id='$id'");
        die;
    }
    ...
    public function moveCeoSetSuccessMessage($succMsg, $ceoData) {
        $this->load->model('Home_model');
        $id = $ceoData['id'];
        $this->db->query("UPDATE moveceo SET message='$succMsg' WHERE id = '$id'");
    }
    ...
    public function moveCompleted($ceoData) {
        $this->load->model('Home_model');
        $id = $ceoData['id'];
        $result = $this->db->query("UPDATE moveceo SET status='success',message='Moved successfully.' WHERE id = '$id'");
        if ($result) {
    ...
    public function changeRedirection($ceoData) {
        ...
        $checkagain = $this->db->query("select * from user where email = '$ceoEmail'");
        if ($checkagain->num_rows() > 0) {
            $toDBbName = $ceoData['toDb'];
            $toDb = "http://$toDBbName";
            $result = $this->db->query("UPDATE user SET redirect='$toDb' WHERE email = '$ceoEmail'");
            if ($result) {
...
//            $old_customer_result['customer_uid'] = $this->generateRandomString();
    ...
    public function updateTime() {
        ...
        if (!$this->db->query("UPDATE domains_info_note SET note_value = '$date' where note_key = 'MoveClientsCron.php' and note_type='crons'")) {
