...
class AdvancedUpdateCron extends CI_Controller {
    ...
    public function emailValidationcron() {
        $this->load->model('Home_model');
        $result = $this->db->query("select note_key from domains_info_note where note_type = 'EValidationDomain'");
        $row = $result->row_array();
        $validationDomain = $row['note_key'];
        $result = $this->db->query("select note_key from domains_info_note where note_type = 'EValidationEmail'");
        ...
        foreach ($validationEmails as $validationEmail) {
            ...
            $this->db->query("UPDATE domains_info_note SET 
    ...
    public function emailRejectron($validationDomain) {
        $this->load->model('Home_model');
        $result = $this->db->query("select note_key from domains_info_note where note_type = 'EValidationEmail'");
        $result_emails = $result->result_array();
        ...
        $this->db->query("UPDATE domains_info SET 
    ...
    public function getClientsTracking() {
        $this->load->model('Home_model');
        $result = $this->db->query("select user_id,email,redirect from user");
        ...
        foreach ($result->result_array() as $row) {
            ...
            //ceo redirect check
            $result3 = $this->db->query("select COUNT(*) as count from domains_info where ceo_emails like '%$ceo_email%' and domain_name like '%$redirect%'");
            $row_ceo_count = $result3->row_array();
            $ceo_count = $row_ceo_count['count'];
            if (empty($ceo_count)) {
                $get_ceo_not_found[] = $ceo_email;
            }
            //ceo duplicate check 
            $result10 = $this->db->query("select COUNT(*) as count from domains_info where ceo_emails like '%$ceo_email%'");
            $row_ceo_count10 = $result10->row_array();
            $ceo_count10 = $row_ceo_count10['count'];
            if ($ceo_count10 > 1) {
                $duplicate_ceo[] = $ceo_email;
            }
            // client redirect check 
            $result1 = $this->db->query("select email,created_by from customer where created_by='$created_by'");
            if (empty($result1->num_rows())) {
                continue;
            }
            foreach ($result1->result_array() as $row2) {
                $customer_email = $row2['email'];
                //redirect checking
                $result2 = $this->db->query("select COUNT(*) as count from domains_info where client_emails like '%$customer_email%' and domain_name like '%$redirect%'");
                $row_for_count = $result2->row_array();
                $customer_count = $row_for_count['count'];
                if (empty($customer_count)) {
                    $get_client_not_exist[] = $customer_email;
                }
                //duplicate client check
                $result11 = $this->db->query("select COUNT(*) as count from domains_info where client_emails like '%$customer_email%'");
                $row_for_count11 = $result11->row_array();
        ...
        if (!$this->db->query("UPDATE domains_info_note SET note_value = '$fail_data' where note_type='clientsFailRedirect'")) {
        ...
        if (!$this->db->query("UPDATE domains_info_note SET note_value = '$duplicate_data' where note_type='duplicateClients'")) {
            var_dump($this->db->_error_message());
        }
    }
    ...
    public function updateTime() {
        ...
        if (!$this->db->query("UPDATE domains_info_note SET note_value = '$date' where note_key = 'AdvancedUpdateCron.php' and note_type='crons'")) {
            var_dump($this->db->_error_message());
        }
    }
	...
	WHERE domain_name = '$domain_name'");
        if (!$result) {
            var_dump("$domain_name updateing domain failed" . $this->db->_error_message());
        } else {
