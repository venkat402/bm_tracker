...
class UnSubscribeEmailsUpdateCron extends CI_Controller {
    ...
    public function trackerExistedEmails() {
        $this->load->model('Home_model');
        $result = $this->db->query("SELECT email FROM email_blacklist");
    ...
    public function insertUnSubscribersToTracker($data) {
        ...
        $result = $this->db->query("INSERT INTO `email_blacklist`(`email_id`,`subscriber_id`,`email`,`reason`,`date_added`,`last_updated`)
    ...
    public function updateTime() {
        ...
        if (!$this->db->query("UPDATE domains_info_note SET note_value = '$date' where note_key = 'UnSubscribeEmailsUpdateCron.php' and note_type='crons'")) {
            var_dump($this->db->error());
        }
    }
